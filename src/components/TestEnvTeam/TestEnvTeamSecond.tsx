...
import TestEnvTeamList from "./TestEnvTeamList";
import "../../styles/TestEnvTeam/TestEnvTeamSecond.scss";
...
export default observer(function (TableList: any) {
  ...
  const isEditObjListChange = (record) =>
    record.headItem.isEditKRs === isEditObjList;
  ...
  const menu = (objItem) => {
    return (
      <Menu>
        <Menu.Item onClick={() => editObjective(objItem)}>
          修改Objective
        </Menu.Item>
              ...
              <div className="contentList" key={objIndex} onClick={() => jump}>
                <div className="listTitle">
                  <div className="objectTitle">
                    <span className="objectTitleNum">O{objIndex + 1}:</span>
                    {objItem.headItem.head}{" "}
                  </div>
                  <div className="objectWeight">
                    权重：{objItem.headItem.weight}%
                  </div>
                  <div className="objectFinish">
                    完成度：{objItem.headItem.finish}%
                  </div>
