...
const EditTable2 = () => {
    ...
    return (<div className="multiRowFormCla">
        <Button type="primary"
            onClick={addRow}>
            添加一行
        </Button>
        ...
        data.map((item : any, index : number) => {
            if (item.edit) {
                return (<div key={
                    ...
                    className="multiRowwrap">
                    <div className="multiRowFormInner">
                        <Input className="rowInp"
                            placeholder={
                                'KR' + (
                                    index + 1
                                )
                            ...
                            onChange={
                                (e) => getTitle(e.target.value, index)
                            }/>

                        <Input className="rowInp" placeholder="权重"
                            value={
                                item.content
                            }
                            onChange={
                                (e) => getContent(e.target.value, index)
                            }/>

                        <Input className="rowInp" placeholder="完成度"
                            value={
                                item.conname
                            }
                            onChange={
                                (e) => getConname(e.target.value, index)
                            }/>
                ...
                return (<div key={
                    ...
                    <div className="multiRowFormInner">
                        ...
                        <div className="rowInp">权重： {
                            item.content
                        }</div>
                        <div className="rowInp">完成度：{
                            item.conname
                        }</div>
                        <Button className="rowInp"
                            onClick={
                                () => deleRow(index)
                        }>删除</Button>
                    </div>
